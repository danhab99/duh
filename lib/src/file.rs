...
impl<F: vfs::FileSystem> Repo<F> {
        ...
        P: FnMut(crate::dedup::DedupProgress),
    {
        ...
        vlog!("repo::stage_file: computing hash for entire file");
        let content_hash = Hash::digest_file_stream(&mut new)?;
        vlog!("repo::stage_file: entire file hash {}", content_hash.to_string());
            ...
            match fragment_res {
                DiffFragment::ADDED { body } => {
                    for chunk in body.chunks(self.max_size) {
                        let frag_hash = self.save_obj(Object::Fragment(Fragment(chunk.to_vec())))?;
                        vlog!(
                            "repo::stage_file: ADDED fragment chunk_size={} -> fragment_hash={}",
                            chunk.len(),
                            frag_hash.to_string()
                        ...
                        let h = self.save_obj(Object::FileDiffFragment(f))?;
                        file_fragments.push(h);
                    }
                }
                DiffFragment::UNCHANGED { len } => {
        ...
        let version_hash = self.save_obj(Object::File(FileStruct {
            content_hash,
            fragments: file_fragments,
        }))?;

        self.index.insert(fp.clone(), version_hash);
        vlog!("repo::stage_file: indexed '{}' -> {}", fp, version_hash.to_string());
    ...
    pub fn staged_summary(&self) -> FileOpsResult<Vec<FileStagedSummary>> {
        ...
        for (path, &version_hash) in &self.index {
            ...
            if let Some(Object::File(file)) = self.get_object(version_hash)? {
                for frag_hash in &file.fragments {
                    if let Some(Object::FileDiffFragment(frag)) = self.get_object(*frag_hash)? {
                        match frag {
                            FileFragment::ADDED { len, .. } => added += len,
                            FileFragment::DELETED { len } => deleted += len,
                            FileFragment::UNCHANGED { len } => unchanged += len,
    ...
    pub fn open_file(&mut self, file_path: String, hash: Hash) -> FileOpsResult<Box<dyn ReadSeek>> {
        ...
        let mut fragments = Vec::with_capacity(file_version.fragments.len());
        for frag_hash in file_version.fragments {
            match self.get_object(frag_hash)? {
                Some(Object::FileDiffFragment(frag)) => fragments.push(frag),
                _ => panic!("expected file diff fragment"),
            }
        }
        ...
        let replay = LazyFileReplay::new(self, parent_reader, fragments)?;
